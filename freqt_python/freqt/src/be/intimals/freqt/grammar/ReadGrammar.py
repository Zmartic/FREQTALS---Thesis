...
def addAttribute(child, abstractNodes_dict, grammar_dict):
    """
     * @param: child, Node
...
def updateAttribute(child, abstractNodes_dict, grammar_dict):
    """
     * @param: child, Node
     * @param: abstractNodes_dict, dictionary with String as keys and list of String as values
     * @param: grammar_dict, dictionary with String as keys and list of String as values
...
def readAttribute(child, abstractNodes_dict, grammar_dict):
    """
    ...
    """
    if child.nodeName in grammar_dict:
        updateAttribute(child, abstractNodes_dict, grammar_dict)
    else:
        addAttribute(child, abstractNodes_dict, grammar_dict)
...
def readSyntheticNodes(root, abstractNodes_dict, grammar_dict):
    """
            ...
            for j in range(len(nodeMap)):  # check if a node i is synthetic
                ...
                if node.name == "synthetic" and str(node.value) == "true":
                    ...
                    for k in range(len(childrenList_list)):  # for each child of Synthetic node
                        if childrenList_list[k].nodeType == Node.ELEMENT_NODE:
                            mandatory = readMandatoryAttribute(childrenList_list[k])
                            ...
                            readAttribute(childrenList_list[k], abstractNodes_dict, grammar_dict)

                    if childrenNodes_list[i].nodeName in syntheticNodes_dict:
                        # find the index of rule, # create new synthetic rule
                        index = findIndex(childrenNodes_list[i].nodeName, syntheticNodes_dict)
...
def readSimpleNode(node, abstractNodes_dict, grammar_dict):
    """
     * @param: node, Node
     * @param: abstractNodes_dict, dictionary with String as keys and list of String as values
     * @param: grammar_dict, dictionary with String as keys and list of String as values
    ...
    for i in range(len(childrenList_list)):
        if childrenList_list[i].hasAttributes() and childrenList_list[i].nodeType == Node.ELEMENT_NODE:
            mandatory = readMandatoryAttribute(childrenList_list[i])
            ...
            readAttribute(childrenList_list[i], abstractNodes_dict, grammar_dict)
...
def readSpecialNode(node, syntheticNodes_dict, grammar_dict):
    """
     * @param: node, Node
    ...
    for i in range(len(childrenNodes_list)):
        if childrenNodes_list[i].hasAttributes() and childrenNodes_list[i].nodeType == Node.ELEMENT_NODE:
            ...
            if adhoc[0] == "Adhoc":  # synthetic child
                ...
                syntheticChildren_dict[childrenNodes_list[i].nodeName] = getRules(syntheticLabel, syntheticNodes_dict)
            else: # normal child
                mandatory = readMandatoryAttribute(childrenNodes_list[i])
...
def readASTNodes(root, abstractNodes_dict, syntheticNodes_dict, grammar_dict):
    """
    ...
    for t in range(len(childrenNodes_list)):  # for each child (AST node)
        ...
        if not childrenNodes_list[t].hasAttributes() and childrenNodes_list[t].hasChildNodes() and childrenNodes_list[t].nodeType == Node.ELEMENT_NODE:
            if checkSyntheticNode(childrenNodes_list[t]):
                readSpecialNode(childrenNodes_list[t], syntheticNodes_dict, grammar_dict)
            else:
                readSimpleNode(childrenNodes_list[t], abstractNodes_dict, grammar_dict)
...
def readGrammar(path, grammar):
    ...
    try:
        ...
        abstractNodes_dict = readAbstractNodes(root)  # dictionary with String as keys and list of String as values
        syntheticNodes_dict = readSyntheticNodes(root, abstractNodes_dict, grammar)  # dictionary with String as keys and list of String as values
        readASTNodes(root, abstractNodes_dict, syntheticNodes_dict, grammar)
